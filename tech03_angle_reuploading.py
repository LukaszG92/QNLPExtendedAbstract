...
def main():
    p = build_common_argparser("(3) Angle encoding + Data re-uploading (L layers)")
    p.add_argument("--L", type=int, default=3, help="Number of re-uploading layers.")
    args = p.parse_args()
    ...
    row = run_and_save(
        technique_name=f"(3) angle + data re-uploading (L={L}) + linear entanglement | readout=Z",
        qnode=qnode,
        ...
        q=cfg.n_qubits,
        L=L,
        entanglement_pattern_label=f"reupload_linear(L={L},{cfg.topology},{cfg.entangler})",
        feature_dim=cfg.n_qubits,
        results_csv=results_csv,
        results_json=results_json,
        preproc_time_s=prep["preproc_time_s"],
        pattern_for_proxy="reupload_linear",
        topology=cfg.topology,
        rot_mode=cfg.rot_mode,
