...
def main():
    ...
    if os.environ.get('QNLP_DEBUG', '0').lower() in ('1','true','yes','y','on'):
        print('[DEBUG] tech01_angle_linear: args=', vars(args))
        print('[DEBUG] tech01_angle_linear: cfg=', cfg)
    ...
    if os.environ.get('QNLP_DEBUG', '0').lower() in ('1','true','yes','y','on'):
        print('[DEBUG] prep shapes:',
              'X_train_ang', prep['X_train_ang'].shape,
              'X_val_ang', prep['X_val_ang'].shape,
              'X_test_ang', prep['X_test_ang'].shape)
    ...
    row = run_and_save(
        technique_name=f"(1) angle + linear entanglement | q={cfg.n_qubits} | L={int(getattr(cfg,'L',1))} | clf={clf_kind}",
        qnode=qnode,
